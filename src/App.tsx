...
const nav: { name: View; icon: string; desc: string }[] = [
  { name: 'Overview', icon: '◈', desc: 'Command center' },
  { name: 'Leads', icon: '◎', desc: 'CRM & pipeline' },
  { name: 'Automations', icon: '✦', desc: 'Workflows' },
  { name: 'AI Chat', icon: '◌', desc: 'Website & social' },
  { name: 'Voice Agent', icon: '◉', desc: 'AI calling' },
  { name: 'Integrations', icon: '⌘', desc: 'Channels & apps' },
  { name: 'Settings', icon: '⚙', desc: 'Workspace' },
];

const integrations = [
  ['Instagram', 'DM automation, comments, story replies', 'Meta'],
  ['WhatsApp', 'Inbound chat, templates, lead routing', 'Meta'],
  ['Google', 'Sign-in, Sheets, Calendar, Drive', 'Google'],
  ['Website Chat', 'Embeddable AI sales assistant', 'AI'],
  ['Voice', 'Outbound calls, qualification, callbacks', 'Telephony'],
  ['Meta Ads', 'Lead forms → CRM → automation', 'Meta'],
];
...
export default function App() {
  ...
  async function loadLeads() {
    setLoading(true);
    const { data } = await supabase.from('leads').select('id,name,company,source_id,estimated_value,status,score,email,phone').order('created_at', { ascending: false }).limit(50);
  ...
  async function signOut() { await supabase.auth.signOut(); setView('Overview'); }

  const filtered = useMemo(() => leads.filter(l => `${l.name} ${l.company ?? ''} ${l.phone ?? ''}`.toLowerCase().includes(query.toLowerCase())), [leads, query]);
  const totalValue = leads.reduce((sum, l) => sum + Number(l.estimated_value || 0), 0);

  if (!session) return <Landing onLogin={() => setLoginOpen(true)} loginOpen={loginOpen} closeLogin={() => setLoginOpen(false)} onGoogle={googleLogin} />;

  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand"><div className="brand-mark">Z</div><div><strong>FlowPilot</strong><span>AI AUTOMATION OS</span></div></div>
      <div className="workspace"><span className="avatar">{(session.user.user_metadata?.full_name || session.user.email || 'U').slice(0,2).toUpperCase()}</span><div><b>{session.user.user_metadata?.full_name || 'My Workspace'}</b><small>{session.user.email}</small></div></div>
      <nav>{nav.map(item => <button key={item.name} className={view === item.name ? 'active' : ''} onClick={() => setView(item.name)}><span>{item.icon}</span><div><b>{item.name}</b><small>{item.desc}</small></div></button>)}</nav>
      <div className="sidebar-bottom"><div className="connection"><i/> Supabase connected</div><button onClick={signOut}>↪ Sign out</button></div>
    </aside>

    <main className="main">
      <header className="topbar"><div><p className="crumb">Workspace / {view}</p><h1>{view}</h1></div><div className="top-actions"><button className="icon">⌕</button><button className="icon">⌁</button><div className="user-avatar">{(session.user.email || 'U')[0].toUpperCase()}</div></div></header>
      {view === 'Overview' && <Overview leads={leads} totalValue={totalValue} onNavigate={setView} />}
      {view === 'Leads' && <Leads leads={filtered} query={query} setQuery={setQuery} loading={loading} onRefresh={loadLeads} onAdd={() => setShowLead(true)} />}
      {view === 'Automations' && <AutomationHub />}
      {view === 'AI Chat' && <AIChat />}
      {view === 'Voice Agent' && <VoiceAgent />}
      {view === 'Integrations' && <Integrations />}
      {view === 'Settings' && <Settings email={session.user.email || ''} />}
      {showLead && <LeadModal close={() => setShowLead(false)} onCreated={() => { setShowLead(false); loadLeads(); }} />}
    </main>
  </div>;
}

function Landing({ onLogin, loginOpen, closeLogin, onGoogle }: { onLogin:()=>void; loginOpen:boolean; closeLogin:()=>void; onGoogle:()=>void }) {
  return <div className="landing">
    <div className="orb orb-one"/><div className="orb orb-two"/>
    <header className="landing-nav"><div className="brand"><div className="brand-mark">Z</div><strong>FlowPilot</strong></div><div className="nav-links"><a href="#features">Features</a><a href="#channels">Channels</a><a href="#agents">AI Agents</a></div><button className="login-top" onClick={onLogin}>Log in <span>↗</span></button></header>
    <section className="hero-section"><div className="hero-copy"><div className="pill"><i/> AI automation platform for modern sales teams</div><h2>One workspace.<br/><em>Every conversation.</em><br/>Fully automated.</h2><p>Capture leads, chat on your website, automate Instagram and WhatsApp, qualify with AI voice calls, and move every opportunity through your pipeline.</p><div className="hero-actions"><button className="primary large" onClick={onLogin}>Start building free <span>→</span></button><button className="ghost large" onClick={onLogin}>See the workspace</button></div><div className="trusted"><span>Built with</span><b>Supabase</b><b>Google</b><b>Meta</b><b>AI</b></div></div><div className="hero-visual"><div className="visual-glow"/><div className="dashboard-float"><div className="mini-head"><span>AI COMMAND CENTER</span><i/></div><div className="mini-number">1,284 <small>active leads</small></div><div className="mini-bars"><i/><i/><i/><i/><i/><i/><i/></div><div className="mini-row"><span>AI calls</span><b>86 today</b></div><div className="mini-row"><span>Automation</span><b className="green">92% healthy</b></div></div><div className="float-card float-chat">✦ <b>AI qualified</b><span>11 hot leads</span></div><div className="float-card float-call">◉ <b>Voice agent</b><span>Calling #204</span></div></div></section>
    <section className="feature-strip" id="features">{[['◎','Real CRM','Every lead, activity and pipeline stage in one place.'],['✦','AI Automations','Build trigger → action workflows without code.'],['◉','Voice Agents','Qualify leads and schedule callbacks automatically.'],['⌘','Omnichannel','Instagram, WhatsApp, web chat and more.']].map(x=><article key={x[1]}><span>{x[0]}</span><h3>{x[1]}</h3><p>{x[2]}</p></article>)}</section>
    {loginOpen && <div className="modal-backdrop" onClick={closeLogin}><div className="login-card" onClick={e=>e.stopPropagation()}><button className="close" onClick={closeLogin}>×</button><div className="brand centered"><div className="brand-mark">Z</div></div><h3>Welcome to FlowPilot</h3><p>Sign in to open your automation workspace.</p><button className="google-btn" onClick={onGoogle}><span>G</span> Continue with Google <b>→</b></button><div className="divider"><span>Secure authentication via Supabase</span></div><small>By continuing, you agree to our terms and privacy policy.</small></div></div>}
  </div>
}

function Overview({ leads, totalValue, onNavigate }: { leads:Lead[]; totalValue:number; onNavigate:(v:View)=>void }) {
  return <div className="page"><section className="welcome"><div><p className="eyebrow">COMMAND CENTER · AUG 10</p><h2>Build, automate, scale. <span>✦</span></h2><p>Your sales operations are connected. Choose an area below to start building.</p></div><button className="primary" onClick={()=>onNavigate('Automations')}>＋ Create automation</button></section><section className="stat-grid">{[['Total leads',String(leads.length),'Live database'],['Hot leads',String(leads.filter(x=>x.score>=80).length),'Score ≥ 80'],['AI calls','0','Connect voice provider'],['Pipeline value',`₹${(totalValue/100000).toFixed(1)}L`,'Live from Supabase']].map(x=><article className="stat" key={x[0]}><span>{x[0]}</span><strong>{x[1]}</strong><p>{x[2]}</p></article>)}</section><section className="section-title"><div><h3>Automation universe</h3><p>Every channel is designed to connect to the same lead database.</p></div></section><div className="module-grid">{[['Automations','Create trigger → action workflows','✦','Automations'],['AI Chat','Build your own sales chatbot','◌','AI Chat'],['Voice Agent','Design an AI calling agent','◉','Voice Agent'],['Integrations','Connect Meta, Google & more','⌘','Integrations']].map(x=><button className="module-card" key={x[0]} onClick={()=>onNavigate(x[3] as View)}><span className="module-icon">{x[2]}</span><h3>{x[0]}</h3><p>{x[1]}</p><b>Open workspace →</b></button>)}</div><section className="health"><div><i/> Platform healthy</div><span>Supabase database · Authentication · RLS enabled</span></section></div>
}

function Leads({ leads, query, setQuery, loading, onRefresh, onAdd }:{leads:Lead[];query:string;setQuery:(v:string)=>void;loading:boolean;onRefresh:()=>void;onAdd:()=>void}) {
  return <div className="page"><div className="page-actions"><div><h2>Leads</h2><p>Real records from your Supabase database.</p></div><button className="primary" onClick={onAdd}>＋ Add lead</button></div><div className="card table-card"><div className="table-tools"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search name, company or phone..."/><button onClick={onRefresh}>↻ Refresh</button></div>{loading?<div className="empty">Loading your leads…</div>:<div className="table-wrap"><table><thead><tr><th>Lead</th><th>Company</th><th>Phone</th><th>Value</th><th>Score</th><th>Status</th></tr></thead><tbody>{leads.map(l=><tr key={l.id}><td><div className="lead-name"><span className="person">{l.name.split(' ').map(x=>x[0]).join('').slice(0,2)}</span><b>{l.name}</b></div></td><td>{l.company || '—'}</td><td>{l.phone || '—'}</td><td>₹{Number(l.estimated_value||0).toLocaleString('en-IN')}</td><td><span className="score">{l.score}</span></td><td><span className={`badge ${l.status}`}>{l.status}</span></td></tr>)}{!leads.length&&<tr><td colSpan={6}><div className="empty">No leads yet. Add your first lead.</div></td></tr>}</tbody></table></div>}</div></div>
}
...
function VoiceAgent(){ return <div className="page"><div className="page-actions"><div><h2>Voice Agent Studio</h2><p>Design an outbound AI caller for qualification, follow-ups and callbacks.</p></div><button className="primary">＋ Create voice agent</button></div><div className="voice-hero"><div className="wave"><i/><i/><i/><i/><i/><i/><i/><i/><i/></div><span>VOICE AGENT</span><h3>AI Sales Caller</h3><p>Hindi · English · Telugu · Marathi</p><div className="voice-actions"><button className="primary">Test a call</button><button className="secondary">Configure</button></div></div><div className="module-grid small">{[['Script','Greeting, qualification & objection handling'],['Voice','Choose provider, voice & language'],['Rules','Call windows, retry policy & outcomes'],['Handoff','Transfer hot leads to your sales team']].map(x=><article className="info-card" key={x[0]}><span>✦</span><h3>{x[0]}</h3><p>{x[1]}</p></article>)}</div></div> }
